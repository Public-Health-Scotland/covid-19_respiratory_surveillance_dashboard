           ...
           tabPanel(stringr::str_to_sentence("Mycoplasma pneumoniae"),
                    ...
                    tagList(h2(glue("Number of acute Mycoplasma pneumoniae admissions to hospital in Scotland")),
                            tags$div(class = "headline",
                                     ...
                                     subtitle = tagList(tags$strong(glue("({format(round_half_up(mpn_admissions_recent_week %>% .$RateTwoWeek,1), nsmall = 1)} per 100,000)")),
                                                        tags$br(),
                                                        glue("Week ending {mpn_admissions_recent_week %>%
                                     ...
                                     valueBox(value = {mpn_admissions_recent_week %>%
                                         ...
                                         subtitle = tagList(tags$strong(glue("({format(round_half_up(mpn_admissions_recent_week %>% .$RateLastWeek,1), nsmall = 1)} per 100,000)")),
                                                            tags$br(),
                                                            glue("Week ending {mpn_admissions_recent_week %>%
                                     ...
                                     valueBox(value = glue("{mpn_admissions_recent_week %>%
                                         .$AdmissionsThisWeek %>% format(big.mark=",")}*"),
                                              subtitle = tagList(tags$strong(glue("({format(round_half_up(mpn_admissions_recent_week %>% .$RateThisWeek,1), nsmall = 1)} per 100,000)")),
                                                                 tags$br(),
                                                                 glue("Week ending {mpn_admissions_recent_week %>%
