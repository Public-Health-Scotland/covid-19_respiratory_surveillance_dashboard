...
rhino_admissions_recent_week <- admissions_scotland %>%
           ...
           tabPanel(stringr::str_to_sentence("Rhinovirus"),
                    ...
                    tagList(h2(glue("Number of acute rhinovirus admissions to hospital in Scotland")),
                            tags$div(class = "headline",
                                     ...
                                     subtitle = tagList(tags$strong(glue("({format(round_half_up(rhino_admissions_recent_week %>% .$RateTwoWeek,1), nsmall = 1)} per 100,000)")),
                                                        tags$br(),
                                                        glue("Week ending {rhino_admissions_recent_week %>%
                                     ...
                                     valueBox(value = {rhino_admissions_recent_week %>%
                                         ...
                                         subtitle = tagList(tags$strong(glue("({format(round_half_up(rhino_admissions_recent_week %>% .$RateLastWeek,1), nsmall = 1)} per 100,000)")),
                                                            tags$br(),
                                                            glue("Week ending {rhino_admissions_recent_week %>%
                                     ...
                                     valueBox(value = glue("{rhino_admissions_recent_week %>%
                                         .$AdmissionsThisWeek %>% format(big.mark=",")}*"),
                                              subtitle = tagList(tags$strong(glue("({format(round_half_up(rhino_admissions_recent_week %>% .$RateThisWeek,1), nsmall = 1)} per 100,000)")),
                                                                 tags$br(),
                                                                 glue("Week ending {rhino_admissions_recent_week %>%
...
fluidRow(width=12, tagList(h2("Rate of acute rhinovirus hospital admissions per 100,000 population in Scotland"))),
...
fluidRow(width=12, tagList(h2("Rate of acute rhinovirus hospital admissions per 100,000 population by age group"))),
