...
#                                     h3(glue("Total number of influenza hospital admissions in Scotland over the last two weeks")),
                                     ...
                                     subtitle = tagList(tags$strong(glue("({format(round_half_up(coron_admissions_recent_week %>% .$RateTwoWeek,1), nsmall = 1)} per 100,000)")),
                                                        tags$br(),
                                                        glue("Week ending {coron_admissions_recent_week %>%
                                     ...
                                     valueBox(value = {coron_admissions_recent_week %>%
                                         ...
                                         subtitle = tagList(tags$strong(glue("({format(round_half_up(coron_admissions_recent_week %>% .$RateLastWeek,1), nsmall = 1)} per 100,000)")),
                                                            tags$br(),
                                                            glue("Week ending {coron_admissions_recent_week %>%
                                     ...
                                     valueBox(value = glue("{coron_admissions_recent_week %>%
                                         .$AdmissionsThisWeek %>% format(big.mark=",")}*"),
                                              subtitle = tagList(tags$strong(glue("({format(round_half_up(coron_admissions_recent_week %>% .$RateThisWeek,1), nsmall = 1)} per 100,000)")),
                                                                 tags$br(),
                                                                 glue("Week ending {coron_admissions_recent_week %>%
...
fluidRow(
  tabBox(width = NULL,
         ...
         tabPanel("Plot",
                  br(),
                  pickerInput(inputId = "adm_season_coron_age",
                              label = "Select season",
