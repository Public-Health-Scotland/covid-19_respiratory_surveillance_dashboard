...
output$health_board_plot =  renderPlotly({
  ...
  p <- plot_ly() %>%
    add_trace(
      ...
      text = ~paste("Week Ending Date:", End, "<br>Average (Mgc/p/d):", round_half_up(average, 2), "<br>Coverage:", paste0(round_half_up(coverage, 2)*100, "%")),
      hoverinfo = "text"
    ) %>%
    ...
    add_trace(
      ...
      name = 'Scotland',
      text = ~paste("Week Ending Date:", End, "<br>Scotland Average (Mgc/p/d):", round_half_up(average, 2), "<br>Scotland Coverage:", paste0(round_half_up(coverage, 2)*100, "%")),
      hoverinfo = "text"
    ) %>%
